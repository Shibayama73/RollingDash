...
//	シールドの出現位置
const Vector3 PlayStage2Scene::SHIELD_UP = Vector3(0.0f, 2.0f, 0.0f);
//	シールドの個数制限
const int PlayStage2Scene::SHIELD_MAX = 5;
//	敵の数制限
const int PlayStage2Scene::ENEMY_MAX = 10;
//	ゲームクリアしたときに一度だけtrueになる
bool PlayStage2Scene::m_justClear = false;
//	ゲームオーバーしたときに一度だけtrueになる
bool PlayStage2Scene::m_justOver = false;
//	ハートのゴールエリアの範囲(円の直径)
const int PlayStage2Scene::GOAL_AREA = 25;

PlayStage2Scene::PlayStage2Scene(int width, int height) :m_outputWidth(width), m_outputHeight(height)
{
}

int PlayStage2Scene::InitializeGame()
{
	...
	m_cubeStage->Initialize(Vector3(0.0f, -15.0f, 0.0f), Vector3(5.0f, 3.0f, 4.0f));
	m_cubeStage->RightRot();
...
int PlayStage2Scene::UpdateGame()
{
	//	追尾カメラの設定
	...
	//	ハートの更新
	m_heart->Update();
	m_heart->ChangePos(Vector3(15.0f, 0.0f, 10.0f));
	...
	m_sceneBase = SceneManager::SCENE::PLAY_STAGE2;
...
void PlayStage2Scene::RenderGame()
{
	m_spriteBatch->Begin();
