...
int Window::Initialize(int width, int height)
{
	...
	//	ウィンドウの生成
	HWND hwnd = CreateWindowEx(0, L"GameWindowClass", L"コロコロダッシュ", WS_OVERLAPPEDWINDOW,
	...
	//SetWindowLongPtr(hwnd, GWLP_USERDATA, reinterpret_cast<LONG_PTR>(g_gameMain.get()));

	//GetClientRect(hwnd, &rc);

	//g_gameMain->Initialize(hwnd, rc.right - rc.left, rc.bottom - rc.top);
